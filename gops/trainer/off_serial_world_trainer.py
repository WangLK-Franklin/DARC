...
class OffSerialWorldTrainer:
    def __init__(self, alg, sampler, buffer:ReplayBuffer, evaluator,disable_diffusion=gin.REQUIRED,trainer_type=gin.REQUIRED, **kwargs):
        ...
        self.diffusion_start = kwargs.get("diffusion_start", 200000)
        self.diffusion_interval = kwargs.get("diffusion_interval", 200000)
    ...
    def step(self):
        ...
        if self.trainer_type == 'diffusion':
            if not self.disable_diffusion and (self.iteration + 1) % self.diffusion_interval == 0 and (self.iteration + 1) >= self.diffusion_start:
