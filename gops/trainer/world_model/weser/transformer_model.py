...
from gops.trainer.world_model.weser.attention_blocks import get_vector_mask
from gops.trainer.world_model.weser.attention_blocks import PositionalEncoding1D, AttentionBlock, AttentionBlockKVCache
...
class StochasticTransformerKVCache(nn.Module):
    def __init__(self, latent_dim, feat_dim, num_layers, num_heads, max_length, dropout):
        super().__init__()
    ...
    def forward_with_kv_cache(self, latent):
        '''
        Forward pass with kv_cache, cache stored in self.kv_cache_list
        '''
        assert latent.shape[1] == 1
        mask = get_vector_mask(self.kv_cache_list[0].shape[1]+1, latent.device)
