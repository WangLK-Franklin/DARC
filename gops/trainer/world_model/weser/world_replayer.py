...
class LinearEncoder(nn.Module):
    def __init__(self, 
                 ...
                 output_activation: nn.Module = nn.Identity):
        super().__init__()
        self.backbone = torch_mlp(sizes, activation, output_activation)
...
class WorldModel(nn.Module):
    ...
    def update(self, obs, action, reward, termination, next_obs, logger=None):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            ...
            embedding = self.encoder(obs)
            # transformer
            temporal_mask = get_subsequent_mask_with_batch_length(batch_length, embedding.device)
            dist_feat = self.storm_transformer(embedding, action, temporal_mask)
            prior_logits = self.dist_head.forward_prior(dist_feat)
