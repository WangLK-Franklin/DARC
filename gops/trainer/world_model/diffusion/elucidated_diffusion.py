...
from world_model.diffusion.norm import BaseNormalizer
...
def make_inputs_from_replay_buffer(
    ...
    obs = replay_buffer.buf["obs"][:ptr_location]
    actions = replay_buffer.buf["act"][:ptr_location]
    next_obs = replay_buffer.buf["obs2"][:ptr_location]
    rewards = replay_buffer.buf["rew"][:ptr_location]
...
class REDQTrainer(Trainer):
    def __init__(
            ...
            ema_decay: float = 0.995,
            adam_betas: Tuple[float, float] = (0.9, 0.99),
            save_and_sample_every: int = 100000000,
            weight_decay: float = 0.,
            results_folder: str = './results',
            amp: bool = False,
            fp16: bool = False,
